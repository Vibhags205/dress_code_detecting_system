...
CAMERA_PROFILE = os.getenv("CAMERA_PROFILE", "laptop").strip().lower()
BASE_THRESHOLD = float(os.getenv("DRESS_THRESHOLD", "0.5"))
PHONE_THRESHOLD = float(os.getenv("DRESS_THRESHOLD_PHONE", str(BASE_THRESHOLD)))
LAPTOP_THRESHOLD = float(os.getenv("DRESS_THRESHOLD_LAPTOP", str(BASE_THRESHOLD)))
ACTIVE_THRESHOLD = PHONE_THRESHOLD if CAMERA_PROFILE == "phone" else LAPTOP_THRESHOLD

# 2. CAMERA SETUP
cap = cv2.VideoCapture(0)
if CAMERA_PROFILE == "phone":
...
# Best-effort controls; unsupported cameras simply ignore these.
cap.set(cv2.CAP_PROP_AUTOFOCUS, 1)
cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.75)

print("--- Dress Code Detector Loaded ---")
print(f"Camera profile: {CAMERA_PROFILE} | Threshold: {ACTIVE_THRESHOLD:.2f}")
print("Controls:")
...
def classify_frame(bgr_frame):
    ...
    # Use multiple views to reduce unstable predictions from noisy cameras.
...
def draw_result_overlay(frame, score, threshold, flipped=False):
    # Determine class based on the 0.5 threshold
    # Default: > 0.5 is Non-Compliant (Alphabetical: Compliance=0, Non-Compliance=1)
    is_nc = (score > threshold) if not flipped else (score <= threshold)
    ...
    cv2.putText(frame, f"Confidence: {confidence:.0%}  score={score:.3f}  thr={threshold:.2f}{flip_note}",
...
while True:
    ...
    if key == ord("l") or key == ord("L"):
        ...
        if state == STATE_RESULT and result_frame_orig is not None:
            ...
            draw_result_overlay(result_frame, score, ACTIVE_THRESHOLD, flipped=labels_flipped)
    ...
    if state == STATE_LIVE:
        ...
        if key == ord(" ") or auto_capture_due: # Manual capture or one-time per person
            ...
            is_nc = (score > ACTIVE_THRESHOLD) if not labels_flipped else (score <= ACTIVE_THRESHOLD)
            result_label = "NON-COMPLIANT" if is_nc else "COMPLIANT"
            confidence = abs(score - ACTIVE_THRESHOLD) * (1.0 / max(ACTIVE_THRESHOLD, 1.0 - ACTIVE_THRESHOLD))
            confidence = max(0.0, min(confidence, 1.0))
